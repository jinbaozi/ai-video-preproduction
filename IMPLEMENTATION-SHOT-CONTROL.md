...
| PR3：关键帧请求、编辑差量、实际生成和审图，身份/场景/姿态可追溯 | 实际母版与多次图片结果、事后回放明确标识、后续事前冻结 stage/receive，以及用途审核 | 闭环工具和真实失败/修复记录存在；可作为首尾帧后端输入的整帧构图仍未通过 |
...
GPT‑6 Pro 网页完成 28 分 9 秒审核，给出 APPROVE_WITHIN_TESTED_PATCH_SCOPE，122/122 项断言通过。B01 原反例在原包/原观察/原媒体下通过；受测 ±8 ULP、配方冻结与超界拒绝成立。宿主 stage/receive/lower 及部分 F01–F03 已补验，但完整联合编译和正式 Skill 安装未运行，因此不宣告旧问题全部关闭。原文和下载摘要见 validation-artifacts/shot-control-audit-a7b4072.md 与对应 summary.json。

审核中途提及的布尔值问题已由本地原生包独立复现并在 6a15aa1 修复；终局报告未将该问题列作 P1/P2。后续标签和类型改动仍需新 SHA 的增量复审，CPU 实验仍维持质量 FAIL。
