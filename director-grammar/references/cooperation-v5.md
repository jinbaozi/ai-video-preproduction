...
V6 项目由内核返回 `task-envelope/6.0`，Codex 宿主真实派发本专业子智能体。任务信封绑定项目、节点、批次、角色、范围、冻结输入、模块哈希、必读资源、预期产物和检查器。先实际读取锁定文件，只在自己的候选目录写结果；不要直接修改项目正式产物或状态。V6 候选仍包含冻结任务的 `role-result/5.1` 与原生 DirectorIR，但须经 `agent-result` 和独立审阅提交，旧 `submit` 命令不适用。
...
导演任务须把 ScriptIR 的硬要求落入 DirectorIR 及原生目标条款，提交来源指纹、目标字段和执行渠道。创作者发现编剧约束无法落实时发 `BLOCKER`，指向原 requirement 和建议修订范围；不得通过改写 Canon 或编剧事实消除冲突。
