...
分镜子智能体只处理信封列出的镜头和冻结的导演、美术、素材版本，在候选目录交 StoryboardIR、检查与逐条来源映射。调度控制包和分镜图片属于图上后续节点；本节点不声称已生图或已执行视频。变更上游锁定项时发结构化阻塞消息交回原责任者。独立审阅和内核检查均通过后才能接受。具体交接见 [协作契约](references/cooperation-v5.md)。
