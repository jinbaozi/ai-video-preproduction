"""Native 1.2 entry points. Old validators and compilers remain untouched."""
...
_spec=importlib.util.spec_from_file_location('spatial_runtime_'+str(Path(__file__).parent),Path(__file__).with_name('spatial_runtime.py'))
...
def compile_video(core,ir,cap,mode,base):
 ...
 # Keep the complete, dimension-separated audit projection. The model-facing
 # projection below is separately ordered by shot and event time.
 for shot in ir['shots']:
  ...
  paths=[f"/shots/{ir['shots'].index(shot)}/{field}" for field in ('start_ms','end_ms','transition','transition_reason','style')]
  ...
  for path in paths:
   for field,value in detail.leaves(detail.pointer(ir,path),path):coverage.append({'source_path':field,'source_sha256':detail.digest(value),'object_id':shot['id'],'disposition':'equivalent_conversion','channel':'prompt','block':index,'text_sha256':detail.digest(text),'reason':'镜头静态制作字段逐项展开'})
 ...
 errors += [core.issue('E_PROMPT_MISSING',path,'必执行字段未进入按镜头正文；完整审计记录仍保留。') for path in gaps]
 errors += verify_prompt(ir,prompt,prompt_coverage,detail)
 mapped={row['source_path'] for row in prompt_coverage}
 contract_rows=[]
 for c in ir['contract']:
  missing=[x['path'] for x in c['checks'] if c['channel']!='post' and x['path'] not in mapped]
  if missing:errors.append(core.issue('E_CONTRACT_PROMPT',missing[0],'硬合同检查字段仅在伴随文件中，不能算作正文覆盖。'))
  contract_rows.append({'id':c['id'],'level':c['level'],'source_refs':c['source_refs'],'ir_paths':[x['path'] for x in c['checks']],
   'channel':c['channel'],'execution':c['execution'],'acceptance':c['acceptance'],'static_assertions':'FAIL' if missing else 'PASS',
   'realization':'NOT_RUN','status':'PLANNED' if c['channel']=='post' else 'BLOCKED' if missing else 'EMITTED'})
 ...
 artifact={'schema':'compiled-artifact/1.3','target':cap['id'],'mode':mode,'status':'BLOCKED' if any(e['severity'] in ('error','blocker') for e in errors) else 'COMPILED',
  ...
  'trace':prompt_trace,'coverage':contract_rows,
  ...
  'detail_blocks':blocks,'detail_coverage':coverage,'prompt_coverage':prompt_coverage,
  'prompt_review':{'status':'PENDING_AGENT_REVIEW','input_sha256':detail.content_hash(ir),
                   'prompt_sha256':sha256(prompt.encode('utf-8')).hexdigest(),'findings':[]},
  'segment_proposals':proposals,'semantic_review':deepcopy(ir['timeline']['semantic_review']),'spatial_checks':detail.geometry_checks(ir)}
 failures=core.schema_errors(artifact,'compile-artifact-1.3')
 if failures:raise ValueError(str(failures))
 return artifact,{'schema':'context-ir/1.2','authoritative_ir_hash':detail.digest(ir),'shots':[{'shot_id':s['id']} for s in ir['shots']],'temporal_hash':detail.temporal_hash(ir),'projection':'complete temporal tracks; no summarization'}
...
def storyboard_package(module,ir,base,out):
 ...
 emit('compiler-handoff.json',{'schema_version':'storyboard-handoff/1.2','project_id':ir['project_id'],'ir_file':'storyboard.ir.json','input_hash':detail.digest(ir),'target':'avir/1.2','source_ids':[x['id'] for x in ir['sources']],'temporal_hash':detail.temporal_hash(ir),'submitted':False})
