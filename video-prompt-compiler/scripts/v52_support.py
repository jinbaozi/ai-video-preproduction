...
def compile_video(core,ir,cap,mode,base):
 ...
 artifact={'schema':'compiled-artifact/1.2','target':cap['id'],'mode':mode,'status':'BLOCKED' if any(e['severity'] in ('error','blocker') for e in errors) else 'COMPILED',
  ...
  'trace':[{'block':i,'text':b['text'],'ir_paths':[b['path']],'source_refs':[],'source_namespace':'avir','rule':'SPATIAL.LOWER.1.2'} for i,b in enumerate(blocks)],
  ...
  'detail_blocks':blocks,'detail_coverage':coverage,'segment_proposals':proposals,'semantic_review':deepcopy(ir['timeline']['semantic_review']),'spatial_checks':detail.geometry_checks(ir)}
 failures=core.schema_errors(artifact,'compile-artifact-1.2')
