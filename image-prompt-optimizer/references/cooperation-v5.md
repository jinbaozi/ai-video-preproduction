...
需要原生制作合同时，使用[制作合同](intent-compiler.md)与本Skill的Schema，作为artifact交回；不得将它冒称为V5任务信封。图像正文区分画内文字与叙事上下文：心理、旁白和台词只有明确要求渲染时才成为图中文字，其他内容仅指导可见表演。静态单图不加入声音、真实运镜或多个先后动作。
...
提示词任务分为视觉资产和分镜图片两个范围，逐项锁定来源、真实参考文件与画面验收条件。图像生成是宿主独立执行任务；提示词候选不得包含虚构的模型调用、实际像素或已看图结论。
