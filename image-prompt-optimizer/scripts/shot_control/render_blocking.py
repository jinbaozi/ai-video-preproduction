...
def svg(frame, view, extent, frames=(), *, label_layout=2):
    ...
    def location(point):
        ...
        return x, y
    centers = [location(p) for p in frame['points']]
    labels = [(x-7, y-7, x+7, y+7) for center in centers if center for x, y in [center]]
    for i, (point, center) in enumerate(zip(frame['points'], centers)):
        if center is None:
            continue
        x, y = center
        ...
        if label_layout == 2:
            # Search nearest rows first; only text moves, never the planned point.
            candidates = sorted(range(16, 345, 18), key=lambda v: abs(v-label_y))
            for candidate in [label_y]+candidates:
                box = (label_x, candidate-14, label_x+text_width, candidate+3)
                if all(box[2]+4 <= a or c+4 <= box[0] or box[3]+3 <= b or d+3 <= box[1]
                       for a, b, c, d in labels):
                    label_y = candidate
                    labels.append(box)
                    break
            else:
                # Dense scenes retain the point and the full legend, not unreadable text.
                continue
            if abs(label_y-y) > 4:
                parts.append(f'<line x1="{x:.3f}" y1="{y:.3f}" x2="{label_x:.3f}" '
                             f'y2="{label_y-5:.3f}" stroke="{color}" stroke-opacity=".5"/>')
...
def review_files(frames, *, label_layout=2):
    ...
    for f in frames:
        if f['shot_id'] not in trajectories:
            ...
            for view in ('top', 'side'):
                rendered = svg(f, view, extent, frames, label_layout=label_layout)
                import re
                trajectories[f['shot_id']][view] = ''.join(re.findall(r'<polyline[^>]*/>', rendered))
    for f in frames:
        slides.append({'shot_id': f['shot_id'], 'at_ms': f['at_ms'],
                       'top': svg(f, 'top', extent, label_layout=label_layout),
                       'side': svg(f, 'side', extent, label_layout=label_layout),
                       'camera': svg(f, 'camera', extent, label_layout=label_layout),
    ...
    if label_layout == 2:
        template = template.replace('<html lang="zh">', '<html lang="zh" data-label-layout="2">')
    ...
    for f in frames:
        if not any(x['shot_id'] == f['shot_id'] and x['at_ms'] < f['at_ms'] for x in frames):
            ...
            index = len(files)
            files[f'review/blocking-{index:03d}.svg'] = svg(f, 'top', extent, frames, label_layout=label_layout)
    return files
