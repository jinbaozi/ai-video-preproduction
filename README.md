...
视频编译器 1.8.0、图片优化器 1.6.0 增加同源调度预览、关键帧请求、编辑差量、事件切点、素材核验、Agnes 模式检查、正文/参数/附件联合编译、按用途失效与返修任务，以及二维媒体观察。使用方法与实装边界见 [镜头控制合同](video-prompt-compiler/references/shot-control.md)。旧项目及 AVIR 版本不迁移，旧 `compile` 输出语义保持；显式使用 `vpc.py control` 启用。
