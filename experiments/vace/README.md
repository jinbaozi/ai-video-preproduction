...
这份真实四秒结果现已进入现有 `review → repair` 工具链，使用完整十二秒 AVIR 的局部时间映射，八个精确抽帧时刻生成 OPEN 返修任务，轨迹覆盖率仍为 0；见[单镜头观察回放](scoped-review-r1/README.md)。失败定位可执行不等于画面已修复。
...
[CPU 65 帧实验](cpu-video-r1/README.md) 已成功执行并保留原始结果，公开四秒 MP4 副本和实际抽帧。全部交付帧仍为卡通画风，质量 FAIL；相较 MPS 的可辨识改善不能归因到单个精度或算子，且无控制输入，不能证明控制收益。耗时 2902.964 秒，商业 API 费用为零，独立服务已在空队列后停止。
