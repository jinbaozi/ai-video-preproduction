...
class V6Runtime:
    ...
    def _reconcile_accepted_compile_review(self) -> None:
        """Do not open another compiler review for a build that V6 already accepted.

        Native ``compile_review.status`` can be cleared while the V6 task for
        that same build is still ``ACCEPTED``. The next ``v5.run()`` would then
        mint a new review task and dispatch it.
        """
        build_id = (self.v5.state.get('build') or {}).get('build_id')
        if not build_id:
            return
        covered = False
        for row in self._stage_rows('compile_review'):
            if row['state'] != 'ACCEPTED':
                continue
            path = self.path('runtime/tasks/'+row['envelope']['task_id']+'.json')
            if not path.is_file():
                continue
            native_task = read(path)
            if (native_task.get('build') or {}).get('build_id') == build_id:
                covered = True
                break
        if not covered:
            return
        ...
        if review.get('status') == 'ACCEPTED' and review.get('build_id') == build_id:
            return
        state['compile_review'] = {'status': 'ACCEPTED', 'build_id': build_id,
                                   'audit': review.get('audit') or 'audited'}
        active_id = state.get('active_task')
        if active_id:
            active_path = self.path('runtime/tasks/'+active_id+'.json')
            if active_path.is_file() and read(active_path).get('kind') == 'compile-review':
    ...
    def _sync_native_invalidations(self) -> None:
        ...
        for row in rows:
            ...
            if node in ('compile', 'compile_review'):
                row_key = (envelope['input_revision'], envelope['batch'])
                newer_reviews = [other for other in rows
                                 if other['state'] == 'ACCEPTED'
                                 and other['envelope']['node_id'] == 'compile_review'
                                 and other['envelope']['scope'] == envelope['scope']
                                 and (other['envelope']['input_revision'],
                                      other['envelope']['batch']) > row_key]
                if newer_reviews:
                    continue
                review = state.get('compile_review') or {}
                if review.get('status') == 'ACCEPTED':
                    compile_review_history.append((
                        f"history/compile-review-invalidated-{_hash({'task_id': envelope['task_id'], 'review': review})}.json",
                        review,
                    ))
                    state['compile_review'] = {**review, 'status': 'INVALIDATED'}
                    changed = True
                continue
        ...
        if changed:
            with transaction(self.root):
                for uri, review in compile_review_history:
                    self.write(uri, review)
                self.v5.save(state)
    ...
    def _ensure_compile_task(self) -> None:
        ...
        previous = self.kernel.snapshot()['tasks'].get(task_id)
        if previous and previous['state'] not in ('FAILED', 'BLOCKED', 'STALE'):
            return
        if previous and previous['state'] == 'FAILED':
            if previous['envelope']['batch'] >= 3 or any(
                    count >= 2 for count in previous['failure_counts'].values()):
    ...
    def _ensure_preproduction_delivery(self) -> None:
        ...
        with transaction(self.root):
            for uri in ('delivery/index.json', 'delivery/index.md'):
                path = self.path(uri)
                if not path.is_file():
                    continue
                digest = digest_file(path)
                archive = f'runtime/v6/evidence/byte-archive/{digest}'
                if not self.path(archive).is_file():
                    self.write(archive, path.read_bytes())
    ...
    def run(self) -> dict:
        ...
        self._reconcile_accepted_compile_review()
        with transaction(self.root):
            result = self.v5.run()
