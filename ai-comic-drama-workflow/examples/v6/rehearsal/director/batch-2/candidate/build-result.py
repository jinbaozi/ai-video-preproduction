...
PREFIX = 'runtime/v6/candidates/TASK_b1e7ff7ce83d7a32bfa09935/2/'
...
assert receipt_state['action_id'] == 'dispatch-170f01b0b05c6d53096de3b2'
assert receipt_state['dispatch_id'] == receipt_state['action_id']
assert receipt_state['batch'] == envelope['batch'] == 2
assert receipt_state['task_digest'] == '3ca9d8e332e93c00178b9e9297894166ba9e996608fd8241bdc9151575c4e364'
assert receipt_state['agent_id'] == '/root/host_bridge/v6_1ca6ab4732989bfd27b839b0'
...
write(EVIDENCE/'director-review.json', {
    ...
    'reviewer':'/root/host_bridge/v6_1ca6ab4732989bfd27b839b0',
    ...
    'batch_1_repair':'已按上一轮独立复核修复所有检查点和镜头终态的甲乙持信文字；对应信件接触和控制权在各时点一致。',
...
expected_states={
    0: ('站立；右手持同一封未拆信','站立；右手准备接信；背包可及',
        ['CHAR_JIA.right_hand'],['CHAR_JIA.right_hand']),
    2500: ('站立；右手仍持同一封未拆信，乙右手已握住另一侧纸边',
           '站立；右手已握住同一封信的另一侧纸边，等待甲松手',
           ['CHAR_JIA.right_hand','CHAR_YI.right_hand'],['CHAR_JIA.right_hand']),
    4000: ('站立；右手已松开并收回，不再持信','站立；右手独自持有同一封未拆信',
           ['CHAR_YI.right_hand'],['CHAR_YI.right_hand']),
    7500: ('站立；右手已松开并收回，不再持信','站立；右手持同一封未拆信，正查看完整封口',
           ['CHAR_YI.right_hand'],['CHAR_YI.right_hand']),
    10500: ('站立；右手已松开并收回，不再持信',
            '站立；右手已松开信，左手在背包开口处；同一封信在包内',
            ['PROP_BACKPACK.interior'],[]),
    12000: ('站立；右手已松开并收回，不再持信',
            '站立；右手已松开信，左手在背包开口处；同一封信在包内',
            ['PROP_BACKPACK.interior'],[]),
}
state_rows=[]
for at,(jia_pose,yi_pose,contacts,controllers) in expected_states.items():
    entities=samples[at]
    actual=(entities['CHAR_JIA']['pose'],entities['CHAR_YI']['pose'],
            entities['PROP_LETTER']['contacts'],entities['PROP_LETTER']['controllers'])
    assert actual == (jia_pose,yi_pose,contacts,controllers), (at,actual)
    state_rows.append({'at_ms':at,'jia_pose':jia_pose,'yi_pose':yi_pose,
                       'letter_contacts':contacts,'letter_controllers':controllers,'status':'PASS'})
end_by_id={row['entity_id']:row for row in ir['shots'][0]['end_state']}
for entity_id in ('CHAR_JIA','CHAR_YI','PROP_LETTER'):
    assert end_by_id[entity_id]['pose'] == samples[12000][entity_id]['pose']
...
write(EVIDENCE/'state-consistency.json', {
    'status':'PASS','review_basis':'上一轮独立复核提出的持信与控制权冲突；本轮逐时点静态语义核对',
    'artifact_uri':rel(ir_path),'artifact_sha256':digest(ir_path),
    'checks':state_rows,'shot_end_matches_12000_ms':True,
...
check_evidence={
    ...
    'native_validator':[evidence_uri('native-validator.json'),evidence_uri('director-review.json'),
                        evidence_uri('state-consistency.json')],
...
candidate={
    ...
    'agent_id':'/root/host_bridge/v6_1ca6ab4732989bfd27b839b0',
