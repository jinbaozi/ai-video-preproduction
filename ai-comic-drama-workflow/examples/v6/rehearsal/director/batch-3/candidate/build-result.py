...
OUT = ROOT / 'runtime/v6/candidates/TASK_b1e7ff7ce83d7a32bfa09935/3'
EVIDENCE = OUT / 'evidence'
PREFIX = 'runtime/v6/candidates/TASK_b1e7ff7ce83d7a32bfa09935/3/'
...
task_state = read(ROOT / 'runtime/v6/state.json')['tasks']['TASK_b1e7ff7ce83d7a32bfa09935']
envelope = task_state['envelope']
receipt_state = task_state['receipt']
assert receipt_state['action_id'] == 'dispatch-5d15b5f4ca205ad31513c0d2'
assert receipt_state['dispatch_id'] == receipt_state['action_id']
assert receipt_state['batch'] == envelope['batch'] == 3
assert receipt_state['task_digest'] == '84becc068a0672dcdb476cc1a9d01e29e7ff5a9592f1c70a657f6f78ac3607e0'
assert receipt_state['agent_id'] == '/root/host_bridge/v6_1af50fa814913a44aa4f78df'
...
repair_brief = read(ROOT / next(item['uri'] for item in envelope['inputs'] if item['slot']=='repair_brief'))
repair_rows=[]
for failure in repair_brief['failures']:
    failure_path=ROOT / failure['failure_uri']
    assert digest(failure_path)==failure['failure_sha256']
    reviews=[]
    for review in failure['review_evidence']:
        review_path=ROOT / review['uri']
        assert digest(review_path)==review['sha256'], review['uri']
        reviews.append({**review, 'verified':True})
    repair_rows.append({'batch':failure['batch'],'failure_uri':failure['failure_uri'],
                        'failure_sha256':failure['failure_sha256'],'reviews':reviews})
...
write(EVIDENCE / 'source-integrity.json', {
    'status':'PASS','action_id':receipt_state['action_id'],'task_id':envelope['task_id'],
    'input_digest':envelope['input_digest'],'input_revision':envelope['input_revision'],
    'inputs':input_rows,'resources':resource_rows,'repair_evidence':repair_rows,
...
write(EVIDENCE/'director-review.json', {
    ...
    'reviewer':'/root/host_bridge/v6_1af50fa814913a44aa4f78df',
    ...
    'batch_1_repair':'各检查点与镜头终态的甲乙姿态符合信件接触和控制权；末态同一封信在背包内。',
    'batch_2_repair':'起态甲右手与信件接触双向一致；交接、独持、验封和收存时的人物、信件及背包接触均双向一致。',
...
expected_states={
    0: ('站立；右手持同一封未拆信','站立；右手准备接信；背包可及',
        ['PROP_LETTER.body'],['PROP_BACKPACK.body'],
        ['CHAR_JIA.right_hand'],['CHAR_JIA.right_hand'],['CHAR_YI.body']),
    2500: ('站立；右手仍持同一封未拆信，乙右手已握住另一侧纸边',
           '站立；右手已握住同一封信的另一侧纸边，等待甲松手',
           ['PROP_LETTER.body'],['PROP_BACKPACK.body','PROP_LETTER.body'],
           ['CHAR_JIA.right_hand','CHAR_YI.right_hand'],['CHAR_JIA.right_hand'],['CHAR_YI.body']),
    4000: ('站立；右手已松开并收回，不再持信','站立；右手独自持有同一封未拆信',
           [],['PROP_BACKPACK.body','PROP_LETTER.body'],
           ['CHAR_YI.right_hand'],['CHAR_YI.right_hand'],['CHAR_YI.body']),
    7500: ('站立；右手已松开并收回，不再持信','站立；右手持同一封未拆信，正查看完整封口',
           [],['PROP_BACKPACK.body','PROP_LETTER.body'],
           ['CHAR_YI.right_hand'],['CHAR_YI.right_hand'],['CHAR_YI.body']),
    10500: ('站立；右手已松开并收回，不再持信',
            '站立；右手已松开信，左手在背包开口处；同一封信在包内',
            [],['PROP_BACKPACK.body','PROP_BACKPACK.opening'],
            ['PROP_BACKPACK.interior'],[],['CHAR_YI.body','CHAR_YI.left_hand','PROP_LETTER.body']),
    12000: ('站立；右手已松开并收回，不再持信',
            '站立；右手已松开信，左手在背包开口处；同一封信在包内',
            [],['PROP_BACKPACK.body','PROP_BACKPACK.opening'],
            ['PROP_BACKPACK.interior'],[],['CHAR_YI.body','CHAR_YI.left_hand','PROP_LETTER.body']),
}
...
state_rows=[]
for at,(jia_pose,yi_pose,jia_contact,yi_contact,letter_contact,letter_controller,bag_contact) in expected_states.items():
    entities=samples[at]
    actual=(entities['CHAR_JIA']['pose'],entities['CHAR_YI']['pose'],
            entities['CHAR_JIA']['contacts'],entities['CHAR_YI']['contacts'],
            entities['PROP_LETTER']['contacts'],entities['PROP_LETTER']['controllers'],
            entities['PROP_BACKPACK']['contacts'])
    expected=(jia_pose,yi_pose,jia_contact,yi_contact,letter_contact,letter_controller,bag_contact)
    assert actual==expected,(at,actual,expected)
    for left,right,left_end,right_end in contact_pairs:
        assert (left_end in entities[left]['contacts']) == (right_end in entities[right]['contacts']), (at,left,right)
    assert len(letter_controller)<=1 and all(controller in letter_contact for controller in letter_controller)
    state_rows.append({'at_ms':at,'jia_pose':jia_pose,'yi_pose':yi_pose,
                       'jia_contacts':jia_contact,'yi_contacts':yi_contact,
                       'letter_contacts':letter_contact,'letter_controllers':letter_controller,
                       'backpack_contacts':bag_contact,'reciprocal_contact_graph':'PASS',
                       'custody':'Jia' if letter_controller==['CHAR_JIA.right_hand'] else
                                 'Yi' if letter_controller==['CHAR_YI.right_hand'] else 'backpack',
                       'status':'PASS'})

# Replay every action delta from the declared initial sample, checking its before value.
changes=sorted((change for action in ir['timeline']['actions'] for change in action['changes']),
...
for at in sorted(samples):
    while index<len(changes) and changes[index]['at_ms']<=at:
        ...
        entity=replay[change['entity_id']]
        assert entity[change['field']]==change['before'], (change['entity_id'],change['field'],change['at_ms'])
        entity[change['field']]=deepcopy(change['after'])
        index+=1
    assert replay==samples[at],at
assert index==len(changes)

for state_name,at in (('start_state',0),('end_state',12000)):
    shot_state={row['entity_id']:row for row in ir['shots'][0][state_name]}
    assert set(shot_state)==set(samples[at])
    for entity_id,detail in samples[at].items():
        legacy=shot_state[entity_id]
        x,y,z=detail['position']
        assert legacy['position_m']==[x,z,y]
        assert legacy['pose']==detail['pose']
        assert legacy['contact']==('；'.join(detail['contacts']) or '无接触')
        assert legacy['support']==('；'.join(detail['supports']) or '不适用')
assert samples[10500]['PROP_LETTER']['pose']==samples[12000]['PROP_LETTER']['pose']=='同一封未拆信在背包内'
assert script['narrative']['reveal_order']==['EVT_HANDOFF','EVT_SEAL_CHECK','EVT_STOW']
write(EVIDENCE/'state-consistency.json', {
    'status':'PASS','review_basis':'两轮独立 handoff 审查的四项具体缺陷；逐时点核对姿态、接触、控制权、动作前态与镜头起止态',
    'artifact_uri':rel(ir_path),'artifact_sha256':digest(ir_path),
    'checks':state_rows,'action_delta_replay':'PASS','all_contact_edges_reciprocal':'PASS',
    'shot_start_matches_0_ms':True,'shot_end_matches_12000_ms':True,
    'single_letter_in_backpack_at_end':True,
    'script_reveal_order':script['narrative']['reveal_order'],
    'independent_professional_review':'PENDING'})
write(EVIDENCE/'repair-audit.json', {
    'status':'PASS','basis':[row['failure_uri'] for row in repair_rows],
    'old_defects':[
        {'batch':1,'id':'CONTRADICTORY_FINAL_CUSTODY','gone_in_batch_3':True,
         'evidence':'10500/12000 ms: Jia no contact, Yi releases letter, one letter contacts backpack interior; shot end matches 12000 ms.'},
        {'batch':1,'id':'STALE_CHECKPOINT_POSES','gone_in_batch_3':True,
         'evidence':'2500/4000/7500/10500/12000 ms poses match letter contact and custody.'},
        {'batch':2,'id':'INITIAL_CONTACT_DENIAL','gone_in_batch_3':True,
         'evidence':'0 ms and shot start: Jia contact is PROP_LETTER.body; letter contact is CHAR_JIA.right_hand.'},
        {'batch':2,'id':'UNMIRRORED_CHARACTER_CONTACTS','gone_in_batch_3':True,
         'evidence':'0/2500/4000/7500/10500/12000 ms person, letter and backpack contacts are reciprocal; actions replay.'}],
...
check_evidence={
    ...
    'native_validator':[evidence_uri('native-validator.json'),evidence_uri('director-review.json'),
                        evidence_uri('state-consistency.json'),evidence_uri('repair-audit.json')],
...
candidate={
    ...
    'agent_id':'/root/host_bridge/v6_1af50fa814913a44aa4f78df',
