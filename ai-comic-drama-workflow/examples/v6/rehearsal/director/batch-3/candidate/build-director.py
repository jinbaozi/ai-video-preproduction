...
initial = {
    'CHAR_JIA': state([-0.65, 0, 0], '站立；右手持同一封未拆信', '信与乙', ['PROP_LETTER.body'], ['双足着地'], [], ['face','eyes','hands','body'], '身份、外观未提供'),
    'CHAR_YI': state([0.65, 0, 0], '站立；右手准备接信；背包可及', '甲手中的信', ['PROP_BACKPACK.body'], ['双足着地'], [], ['face','eyes','hands','body'], '身份、外观未提供'),
...
actions = [
    ...
    action('ACT_GRIP', 2000, 2800, 'GROUP_HANDOFF', 'EVT_HANDOFF', 'CHAR_YI', 'PROP_LETTER',
           ...
           [change('CHAR_JIA','pose','站立；右手持同一封未拆信','站立；右手仍持同一封未拆信，乙右手已握住另一侧纸边',2500),
            ...
            change('CHAR_YI','contacts',['PROP_BACKPACK.body'],['PROP_BACKPACK.body','PROP_LETTER.body'],2500),
    ...
    action('ACT_RELEASE', 2800, 4000, 'GROUP_HANDOFF', 'EVT_HANDOFF', 'CHAR_JIA', 'PROP_LETTER',
           ...
           [change('CHAR_JIA','pose','站立；右手仍持同一封未拆信，乙右手已握住另一侧纸边','站立；右手已松开并收回，不再持信',3500),
            change('CHAR_JIA','contacts',['PROP_LETTER.body'],[],3500),
    ...
    action('ACT_STOW', 8000, 11000, 'GROUP_STOW', 'EVT_STOW', 'CHAR_YI', 'PROP_BACKPACK',
           ...
           [change('CHAR_YI','gaze','同一封信的封口','背包开口',8500),
            ...
            change('CHAR_YI','contacts',['PROP_BACKPACK.body','PROP_LETTER.body'],['PROP_BACKPACK.body','PROP_BACKPACK.opening','PROP_LETTER.body'],8500),
            change('PROP_BACKPACK','contacts',['CHAR_YI.body'],['CHAR_YI.body','CHAR_YI.left_hand'],8500),
            change('CHAR_JIA','gaze','信与乙','乙的收存动作与背包开口',8500),
            change('CHAR_YI','pose','站立；右手持同一封未拆信移向背包开口，左手保持开口','站立；右手已松开信，左手在背包开口处；同一封信在包内',10500),
            change('CHAR_YI','contacts',['PROP_BACKPACK.body','PROP_BACKPACK.opening','PROP_LETTER.body'],['PROP_BACKPACK.body','PROP_BACKPACK.opening'],10500),
            change('PROP_BACKPACK','contacts',['CHAR_YI.body','CHAR_YI.left_hand'],['CHAR_YI.body','CHAR_YI.left_hand','PROP_LETTER.body'],10500),
...
ir={
    'schema_version':'1.2','project_id':'LETTER_DEMO','revision':3,'canon_ref':str(CANON),
    'sources':sources,
...
ir['timeline']['semantic_review']={
    'status':'PASS','reviewer':'/root/host_bridge/v6_1af50fa814913a44aa4f78df / current-agent source-bound review',
    ...
    'findings':[
        ...
        '六个状态样本、镜头起止态和动作变更同步反映交信、双手接触、乙独持、验封、入包；人物与道具接触双向一致。',
...
stamp={'screenplay_sha256':handoff['content_sha256'],
       ...
       'reviewer':'/root/host_bridge/v6_1af50fa814913a44aa4f78df / current-agent screenplay mapping review',
       'findings':['逐项核对六条冻结编剧要求的 ID、源指纹与原生 DirectorIR 字段。',
                   ...
                   'batch 3 对所有样本和起止态核对信件归属、人物姿态及双向接触，修复前两轮独立审查指出的矛盾。']}
rows=[]
for req in required:
