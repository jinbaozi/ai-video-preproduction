"""Validate and package the repaired Director candidate without formal state changes."""
...
PREFIX = 'runtime/v6/candidates/TASK_19a3cd14f25bf90e50215d4d/1/'
MODULE = ROOT / 'runtime/modules/0cac239790b608f91542f2902607f91d444ab246be102a4b062448144d08fa64/director-grammar'
V5_TASK = ROOT / 'runtime/tasks/TASK_19a3cd14f25bf90e50215d4d.json'
...
state = read(ROOT / 'runtime/v6/state.json')['tasks']['TASK_19a3cd14f25bf90e50215d4d']
envelope = state['envelope']
receipt = state['receipt']
task = read(V5_TASK)
assert state['state'] == 'RUNNING'
assert envelope['batch'] == receipt['batch'] == 1
assert receipt['action_id'] == receipt['dispatch_id'] == 'dispatch-b2675e0a86b84995c72fed2c'
...
# Check the Compile review defect in the repaired candidate. DirectorIR 1.2
# mandates two axis orders; assert their explicit conversion at every
# boundary and one time-axis coordinate in downstream-facing text.
old_ir = read(OLD / 'director-ir.json')
old_camera = old_ir['shots'][0]['camera']
new_camera = ir['shots'][0]['camera']
old_operation = old_ir['timeline']['camera_operations'][0]
new_operation = ir['timeline']['camera_operations'][0]
assert old_camera['start_m'] == old_camera['end_m'] == [0, -2.8, 1.2]
assert old_operation['start_position'] == old_operation['end_position'] == '双人前方同侧示意机位 [0,-2.8,1.2] 米'
...
assert new_camera['start_m'] == new_camera['end_m'] == [0, -2.8, 1.2]
assert new_camera['look_at_m'] == [0, 0, 1.0]
assert [new_camera['start_m'][0], new_camera['start_m'][2], new_camera['start_m'][1]] == [0, 1.2, -2.8]
assert [new_camera['look_at_m'][0], new_camera['look_at_m'][2], new_camera['look_at_m'][1]] == [0, 1.0, 0]
assert new_operation['start_position'] == new_operation['end_position'] == '双人前方同侧示意机位 [0,1.2,-2.8] 米（时间轴坐标：x右、y上、z深）'
assert new_camera['movement'] == 'locked' and new_operation['operation'] == 'locked'
for key, at in (('start_state', 0), ('end_state', 12000)):
    by_id = {row['entity_id']: row['position_m'] for row in ir['shots'][0][key]}
    for entity_id, sample in samples[at].items():
        x, y, z = sample['position']
        assert by_id[entity_id] == [x, z, y]
assert len(ir['timeline']['state_samples']) == 6
assert '[0,-2.8,1.2]' not in str(ir['timeline']['camera_operations'])
write(EVIDENCE / 'coordinate-repair.json', {
    'status': 'PASS', 'error_code': 'CAMERA_COORDINATE_CONFLICT',
    'upstream_repair_evidence_uri': evidence('upstream-repair.json'),
    'old_director_uri': relative(OLD / 'director-ir.json'),
    'old_director_sha256': file_hash(OLD / 'director-ir.json'),
    'new_director_uri': relative(ir_path), 'new_director_sha256': file_hash(ir_path),
    'static_axes': 'X-right,Y-depth,Z-up',
    'timeline_axes': 'x-right,y-up,z-depth',
    'conversion_static_to_timeline': '[x,y,z] -> [x,z,y]',
    'static_camera_at_0_and_12000_ms_m': new_camera['start_m'],
    'timeline_camera_at_0_and_12000_ms_m': [0, 1.2, -2.8],
    'new_camera_operation_text': new_operation['start_position'],
    'static_look_at_m': new_camera['look_at_m'],
    'timeline_look_at_m': [0, 1.0, 0],
    'all_six_samples_checked': True,
    'shot_boundary_positions_convert_to_timeline': True,
    'failure_corrected_in_director_candidate': True,
    'downstream_rebuild_and_independent_review': 'PENDING',
})
...
for item in envelope['handoffs']:
    ...
    v6_handoffs.append({**item, 'source_uri': source['uri'], 'source_sha256': source['sha256'],
                        ...
                        'evidence': [evidence('source-integrity.json'), evidence('v5-handoff.json'),
                                     evidence('state-consistency.json'), evidence('coordinate-repair.json')]})
