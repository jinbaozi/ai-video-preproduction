...
TASK_ID = 'TASK_536f58cce58a7823bdf4a3f3'
...
assert TASK['review_dispatch']['agent_id'] == '/root/host_bridge/v6_e6d5d7272377d7fa95042b27'
assert TASK['review_dispatch']['dispatch_id'] == 'review-ae36c50928a68e33a352bda0'
...
# Verify the two explicitly declared Director coordinate bases, then Art's static scene basis.
static_basis = DIRECTOR['scenes'][0]['coordinate_system']
timeline_basis = DIRECTOR['timeline']['coordinate_system']
assert static_basis == 'right-handed:X-right,Y-depth,Z-up;meters'
assert timeline_basis == 'x-right,y-up,z-depth; screen and anatomical directions are separate'
assert ART['set']['coordinate_system'] == static_basis
assert V5_TASK['handoff']['inputs'][0]['coordinates']['position_transform'] == '[x,y,z] -> [x,z,y]'
static_camera = DIRECTOR['shots'][0]['camera']
timeline_camera = DIRECTOR['timeline']['camera_operations'][0]
def triple(value):
    match = re.search(r'\[([^]]+)\]', value)
    assert match and '时间轴坐标' in value
    return [float(part) for part in match.group(1).split(',')]
def transform(value):
    return [value[0], value[2], value[1]]
assert static_camera['start_m'] == static_camera['end_m'] == [0, -2.8, 1.2]
assert triple(timeline_camera['start_position']) == transform(static_camera['start_m'])
assert triple(timeline_camera['end_position']) == transform(static_camera['end_m'])
compiled_prompt = (CANDIDATE / 'compiled-generic-video/prompts/S1.txt').read_text()
assert '场景坐标：' + static_basis in compiled_prompt.splitlines()[6]
assert '0,-2.8' not in compiled_prompt and '0,1.2' not in compiled_prompt
assert ART['sources'][2]['claim'].find('X-right,Y-depth,Z-up') >= 0

# The native V5 Director content differs only in documented source URI and semantic-review rebinding.
native_director = json.loads(Path(integrity['binding']['director_native_uri']).read_text())
def differing_paths(left, right, path=''):
    ...
    if isinstance(left, dict):
        if left.keys() != right.keys():
            return [path]
        return [diff for key in left for diff in differing_paths(left[key], right[key], path + '/' + key)]
    if isinstance(left, list):
        if len(left) != len(right):
            return [path]
        return [diff for i, (l, r) in enumerate(zip(left, right)) for diff in differing_paths(l, r, path + '/' + str(i))]
    return [path] if left != right else []
rebase_paths = sorted(differing_paths(DIRECTOR, native_director))
assert rebase_paths == sorted(integrity['binding']['director_native_rebase_fields'])
assert integrity['binding']['director_native_content_equivalent_after_provenance_rebase'] is True
...
handoff_review = save('handoff.review.json', {
    ...
    'coordinate_basis': {
        'director_static_scene_basis': static_basis,
        'director_static_camera_start_m': static_camera['start_m'],
        'director_static_camera_end_m': static_camera['end_m'],
        'director_timeline_basis': timeline_basis,
        'director_timeline_camera_start_position': timeline_camera['start_position'],
        'director_timeline_camera_end_position': timeline_camera['end_position'],
        'transform_static_to_timeline': '[x,y,z] -> [x,z,y]',
        'art_set_basis': ART['set']['coordinate_system'],
        'compiled_prompt_uri': str((CANDIDATE / 'compiled-generic-video/prompts/S1.txt').relative_to(ROOT)),
        'compiled_prompt_line': 7,
        'compiled_prompt_line_text': compiled_prompt.splitlines()[6],
        'camera_numbers_rewritten_by_art': False,
        'conclusion': 'The two Director arrays are equivalent in separately declared bases; Art uses the static scene basis and does not introduce a camera coordinate triple.'
    },
    'v5_director_rebase_only_paths': rebase_paths,
})
