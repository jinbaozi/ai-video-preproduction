...
REL = 'runtime/v6/candidates/TASK_bf0865334b4a88be570a164a/1'
OUT = ROOT / REL
...
OLD = ROOT / 'runtime/v6/candidates/TASK_eb6702e9e268b2ba339b0fcf/3'
...
text = text.replace('design://LETTER_FULL_DEMO/director-v2', 'design://LETTER_FULL_DEMO/director-current-b1')
ir = json.loads(text)
ir['revision'] = 1
...
findings = [
    ...
    '三镜九秒、机位和左右站位属于本批导演设计；具体地点、室内外、角色外观及背包样式仍未指定。静态设计未作为实际媒体审片。',
]
ir['timeline']['semantic_review'] = {'status': 'PASS', 'reviewer': 'current-agent / TASK_bf0865334b4a88be570a164a / batch 1', 'input_sha256': detail_hash(ir), 'findings': findings}
...
map_data = {'schema': 'screenplay-director-map/1.0', 'screenplay_sha256': handoff['content_sha256'], 'director_sha256': screenplay_hash(ir), 'reviewer': 'current-agent / TASK_bf0865334b4a88be570a164a / batch 1', 'findings': findings, 'mappings': mappings}
...
cmd = ['python3.12', str(MOD / 'scripts/dg.py'), 'validate', str(ir_path), '--screenplay-handoff', str(SCRIPT_HANDOFF), '--screenplay-map', str(map_path)]
...
checks = [
    ...
    {'id': 'native_validator', 'status': 'PASS', 'finding': '锁定 DirectorIR 1.2 原生校验返回 STATIC_VALID；编剧绑定 REVIEW_ATTESTED；终态显式样本信在包内不外露。', 'evidence': [REL + '/evidence/native-validator.json', REL + '/evidence/stow-continuity.json']},
