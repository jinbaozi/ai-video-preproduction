"""Read-only independent audit of the frozen Director candidate."""
...
REVIEW = ROOT / "runtime/v6/reviews" / TASK / "1"
...
conflict = (
    camera["end_ms"] == 9000
    and "信" in camera["end_framing"] and "同框" in camera["end_framing"]
    and "N_PROP_LETTER" not in subjects
    and all(item["letter_contacts"] == ["PROP_BACKPACK.interior"] for item in stow if item["at_ms"] >= 8500)
)
...
native_uri = write("native-validator.review.json", {
    ...
    "finding": "At 9000 ms /timeline/camera_operations/2/end_framing still asks for 乙手、信和背包口同框. The 8500-9000 ms settled composition omits the letter, and explicit state samples put it inside the backpack. Native schema validation misses this semantic conflict.",
    "repair": "Change only /timeline/camera_operations/2/end_framing to show the settled frame: 乙手与背包口可见，信已完全在包内且不外露. Rerun the locked validator and independent review.",
    "media": "NOT_RUN",
})
...
art_pointer_pass = (
    ...
    and art_sha == "6e83e075da45785541fce62f9fcc6ebc8666f48466c382af5980f292613fb5f9"
...
handoff_uri = write("handoff.review.json", {
    ...
    "limitation": "The separate terminal camera framing conflict is recorded under native_validator.",
})
...
record = {
    ...
    "task_id": TASK,
    "batch": 1,
    "reviewer_agent_id": "/root/host_bridge/full_rehearsal/v6_409b607d1ac2769916e7252e",
    "reviewer_dispatch_id": "review-13e0650838434e8bca4ce7df",
    "candidate_digest": candidate_digest(candidate),
    "checks": [
        {"id": "module_receipt", "status": "PASS" if module_pass else "FAIL", "evidence": [module_uri]},
        {"id": "native_validator", "status": "FAIL", "evidence": [native_uri, PREFIX + "native-validator.raw.json"]},
        {"id": "handoff", "status": "PASS" if handoff_pass else "FAIL", "evidence": [handoff_uri]},
    ],
    "failure_owner": "director",
    "at": datetime.datetime.now(datetime.timezone.utc).isoformat().replace("+00:00", "Z"),
}
write("review-record.json", record)
print(json.dumps({"review_record": str(REVIEW / "review-record.json"), "review_sha256": digest(REVIEW / "review-record.json"), "candidate_digest": candidate_digest(candidate), "checks": [(item["id"], item["status"]) for item in record["checks"]], "framing_conflict": conflict}, ensure_ascii=False, indent=2))
