...
REL = 'runtime/v6/candidates/TASK_eb6702e9e268b2ba339b0fcf/3'
OUT = ROOT / REL
...
handoff_check = {'status': 'PASS', 'canon_source_requirements': len(canon_mappings),
                 ...
                 'media': 'NOT_RUN'}
handoff_check['dispatch_correction'] = {
    'original_dispatch_text_uri': '/private/tmp/ai-video-v6-full-demo/full-director-b3-spawn-actual-message.txt',
    'original_dispatch_text_sha256': filehash(Path('/private/tmp/ai-video-v6-full-demo/full-director-b3-spawn-actual-message.txt')),
    'registered_correction_message_id': 'FULL_DIRECTOR_B3_REPAIR_INSTRUCTION_1',
    'registered_correction_subject_uri': 'runtime/v6/host-messages/full-director-b3-repair-instruction.txt',
    'registered_correction_subject_sha256': filehash(ROOT / 'runtime/v6/host-messages/full-director-b3-repair-instruction.txt'),
    'correct_resource_uri': MODREL + '/references/screenplay-handoff.md',
    'correct_resource_sha256': filehash(MOD / 'references/screenplay-handoff.md'),
}
assert handoff_check['dispatch_correction']['original_dispatch_text_sha256'] == 'be0fbbf7ed11238b7ad7062b163faa7c9ef23b390f0ef977d594199d50e908c4'
assert handoff_check['dispatch_correction']['registered_correction_subject_sha256'] == '4c7e647494729fbdb7505e47b393aad4e184c369aba896b6f32baf07accc51e0'
assert handoff_check['dispatch_correction']['correct_resource_sha256'] == 'bc7faaecb662bfed362cbe94697c1d7cfffa813f3f58b79c156c29d1b9344504'
handoff_check['repair_history'] = [
    {'prior_batch': 1, 'failure_uri': 'runtime/v6/failures/TASK_eb6702e9e268b2ba339b0fcf/1/0e276ea9e87a7e82db3a8a4624acf10287969bb241527b72d32e4916fb6c8eea.json', 'defect': 'terminal letter still visibly framed after stow', 'resolution': 'PASS; explicit 8500 and 9000 ms settled state and composition hide letter; see stow-continuity.json'},
    {'prior_batch': 2, 'failure_uri': 'runtime/v6/failures/TASK_eb6702e9e268b2ba339b0fcf/2/e4ee8fd8222294abde4440b2870100925d62bc54185aca5708b173916144461b.json', 'defect': 'Inconsistent screenplay review binding', 'resolution': 'PASS; six native screenplay mapping rows use identical review stamp; V6 candidate retains two separate artifact handoffs'},
]
...
native_stamp = {key: deepcopy(MAP[key]) for key in ('screenplay_sha256', 'director_sha256', 'reviewer', 'findings')}
assert len(MAP['mappings']) == 6
assert native_stamp['reviewer'].endswith('/ batch 3')
native_handoff = [{**deepcopy(row), 'review': deepcopy(native_stamp)} for row in MAP['mappings']]
assert len({json.dumps(row['review'], ensure_ascii=False, sort_keys=True) for row in native_handoff}) == 1
assert {row['requirement_id'] for row in native_handoff} == set(required)
assert {row['requirement_id'] for row in handoffs} == {'UP_9574ea9e1502535dcd879c58', 'UP_84941d3b7d9370ab514cb086'}
...
role_result = {'schema': 'role-result/5.1', 'task_id': 'TASK_eb6702e9e268b2ba339b0fcf',
    ...
    'conflicts': [], 'unresolved': [], 'handoff': native_handoff,
...
candidate = {'schema': 'candidate-result/6.0', 'task_id': role_result['task_id'], 'batch': 3,
    'agent_id': '/root/host_bridge/full_rehearsal/v6_19f7d0afd51ff59e575a24ec',
    'input_digest': '7f690573f3f747f4c4490f755d1f52b14a7c2c235e449f4f8d0564ad152d38a4',
