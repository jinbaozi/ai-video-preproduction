    ...
    with tempfile.TemporaryDirectory(prefix='v5-isolated-') as temporary:
        ...
        for name,pattern in zip(NAMES,PATTERNS):
            ...
            if name=='video-prompt-compiler':
                for pattern in ('test_joint_control.py','test_control_repair.py','test_keyframe_handoff.py','test_previs.py','test_craft.py','test_control_audit_round2.py','test_prompt_projection.py'):
