...
五个专业技能各自自包含。总工作流的assets/bundled-skills保存它们的标准发行包，modules.lock.json锁版本和SHA-256。
...
在开发工作区执行 `python scripts/package_suite.py --workspace <六个源码的父目录> --out <发行目录>`。
它从五份专业源码构建独立包，将同一包字节放入总包，再构建总工作流包。没有第二套人工专业规则。
...
当前活动集合固定六个Skill；video-storyboard-prompter-zh保留历史源码和发行包，不纳入新套装。
